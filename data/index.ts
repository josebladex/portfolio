export const HeroData = {
  es: [
    {
      words: [
        '¡Hola! Soy José Plata.',
        'un desarrollador Full Stack',
        'y Estudiante de Ingeniería Mecatrónica.'
      ],
      buttonTitle: 'Curriculum Vitae'
    }
  ],
  en: [
    {
      words: [
        "Hi! I'm José Plata.",
        'a Full Stack Developer',
        'and Mechatronics Engineering Student.'
      ],
      buttonTitle: 'Resume Jose Plata'
    }
  ]
};

export const projects = {
  es: [
    {
      id: 1,
      title: 'Pixl Technical Test - Plataforma de Eventos',
      des: 'Plataforma para eventos con autenticación y pagos integrados.',
      img1: '/projects/20.png',
      img2: '/projects/21.png',
      iconLists: [
        '/next.svg',
        '/ts.svg',
        '/bg/postgresql.svg',
        '/prisma.svg',
        '/docker-svgrepo-com.svg'
      ],
      link: 'https://github.com/josebladex/pixl-test',
      year: '2025'
    },
    {
      id: 2,
      title: 'SCHNEIDER ELECTRIC Hackathon 2025',
      des: 'Ciencia de datos para optimizar procesos industriales.',
      img1: '/projects/19.png',
      img2: '/projects/19.png',
      iconLists: ['/bg/python-5.svg'],
      link: 'https://nuwe.io/es/hackathons/schneider-electric-iberian-2025?section=challenges',
      year: '2025'
    },
    {
      id: 3,
      title: 'SchedSyncAI',
      des: 'SaaS para sincronizar agendas con inteligencia artificial.',
      img1: '/projects/18.png',
      img2: '/projects/17.png',
      iconLists: ['/docker-svgrepo-com.svg'],
      link: '#',
      year: '?'
    },
    {
      id: 4,
      title: 'Logistica MRMS',
      des: 'Gestión de transporte y logística con colaboración empresarial.',
      img1: '/projects/14.png',
      img2: '/projects/13.png',
      iconLists: ['/re.svg', '/docker-svgrepo-com.svg', '/bg/python-5.svg'],
      link: 'https://logisticamrms.com/',
      year: '2025'
    },
    {
      id: 5,
      title: 'Hackathon: The Hack is ON',
      des: 'Solución Full Stack para un reto técnico.',
      img1: '/projects/16.png',
      img2: '/projects/15.png',
      iconLists: ['/re.svg', '/java.svg', '/spring.svg'],
      link: 'https://nuwe.io/es/hackathons/the-hack-is-on',
      year: '2025'
    },
    {
      id: 6,
      title: 'BOOST HACKATHON',
      des: 'Ciencia de datos y machine learning para optimización empresarial.',
      img1: '/projects/12.png',
      img2: '/projects/11.png',
      iconLists: ['/bg/python-5.svg'],
      link: 'https://nuwe.io/es/hackathons/boost-hackathon',
      year: '2024'
    },
    {
      id: 7,
      title: 'UTOPIA',
      des: 'Explorador de exoplanetas para el desafío NASA Space Apps.',
      img1: '/projects/9.png',
      img2: '/projects/10.png',
      iconLists: ['/three.svg', '/next.svg', '/bg/python-5.svg', '/ts.svg'],
      link: 'https://utopia-exoplanet-quest.vercel.app/',
      year: '2024'
    },
    {
      id: 8,
      title: 'Mundo Pie',
      des: 'Aplicación web para marketing y CRM clínico.',
      img1: '/projects/1.png',
      img2: '/projects/8.png',
      iconLists: [
        '/next.svg',
        '/ts.svg',
        '/bg/postgresql.svg',
        '/aws.svg',
        '/bg/google-calendar-svgrepo-com.svg'
      ],
      link: 'https://mundopie.cl/',
      year: '2023'
    },
    {
      id: 9,
      title: 'Electromagnetismo',
      des: 'Programas para analizar experimentos de electromagnetismo.',
      img1: '/projects/2.png',
      img2: '/projects/3.png',
      iconLists: ['/bg/python-5.svg', '/bg/arduino-1.svg'],
      link: 'https://github.com/josebladex/Recursos-Python-Fisica-II',
      year: '2022'
    },
    {
      id: 10,
      title: 'EnriqueLeeApp',
      des: 'Plataforma de red social con soporte móvil.',
      img1: '/projects/4.png',
      img2: '/projects/5.png',
      iconLists: [
        '/bg/android-svgrepo-com.svg',
        '/bg/kotlin-svgrepo-com.svg',
        '/bg/firebase-svgrepo-com.svg'
      ],
      link: 'https://github.com/josebladex/EnriqueLeeApp',
      year: '2022'
    },
    {
      id: 11,
      title: 'Gotarget',
      des: 'Proyecto inicial de landing page para marketing.',
      img1: '/projects/6.png',
      img2: '/projects/7.png',
      iconLists: [
        '/bg/html-5-svgrepo-com.svg',
        '/bg/css-3-svgrepo-com.svg',
        '/bg/javascript-svgrepo-com.svg'
      ],
      link: 'https://www.youtube.com/watch?v=kfFfxlA0w-A',
      year: '2016'
    }
  ],
  en: [
    {
      id: 1,
      title: 'Pixl Technical Test - Event Management Platform',
      des: 'Platform for events with authentication and integrated payments.',
      img1: '/projects/20.png',
      img2: '/projects/21.png',
      iconLists: [
        '/next.svg',
        '/ts.svg',
        '/bg/postgresql.svg',
        '/prisma.svg',
        '/docker-svgrepo-com.svg'
      ],
      link: 'https://github.com/josebladex/pixl-test',
      year: '2025'
    },
    {
      id: 2,
      title: 'SCHNEIDER ELECTRIC Hackathon 2025',
      des: 'Data science to optimize industrial processes.',
      img1: '/projects/19.png',
      img2: '/projects/19.png',
      iconLists: ['/bg/python-5.svg'],
      link: 'https://nuwe.io/es/hackathons/schneider-electric-iberian-2025?section=challenges',
      year: '2025'
    },
    {
      id: 3,
      title: 'SchedSyncAI',
      des: 'SaaS to synchronize schedules with artificial intelligence.',
      img1: '/projects/18.png',
      img2: '/projects/17.png',
      iconLists: ['/docker-svgrepo-com.svg'],
      link: '#',
      year: '?'
    },
    {
      id: 4,
      title: 'Logistica MRMS',
      des: 'Transportation and logistics management with business collaboration.',
      img1: '/projects/14.png',
      img2: '/projects/13.png',
      iconLists: ['/re.svg', '/docker-svgrepo-com.svg', '/bg/python-5.svg'],
      link: 'https://logisticamrms.com/',
      year: '2025'
    },
    {
      id: 5,
      title: 'Hackathon: The Hack is ON',
      des: 'Full Stack solution for a technical challenge.',
      img1: '/projects/16.png',
      img2: '/projects/15.png',
      iconLists: ['/re.svg', '/java.svg', '/spring.svg'],
      link: 'https://nuwe.io/es/hackathons/the-hack-is-on',
      year: '2025'
    },
    {
      id: 6,
      title: 'BOOST HACKATHON',
      des: 'Data science and machine learning for business optimization.',
      img1: '/projects/12.png',
      img2: '/projects/11.png',
      iconLists: ['/bg/python-5.svg'],
      link: 'https://nuwe.io/es/hackathons/boost-hackathon',
      year: '2024'
    },
    {
      id: 7,
      title: 'UTOPIA',
      des: 'Exoplanet explorer for NASA Space Apps challenge.',
      img1: '/projects/9.png',
      img2: '/projects/10.png',
      iconLists: ['/three.svg', '/next.svg', '/bg/python-5.svg', '/ts.svg'],
      link: 'https://utopia-exoplanet-quest.vercel.app/',
      year: '2024'
    },
    {
      id: 8,
      title: 'Mundo Pie',
      des: 'Web application for marketing and clinical CRM.',
      img1: '/projects/1.png',
      img2: '/projects/8.png',
      iconLists: [
        '/next.svg',
        '/ts.svg',
        '/bg/postgresql.svg',
        '/aws.svg',
        '/bg/google-calendar-svgrepo-com.svg'
      ],
      link: 'https://mundopie.cl/',
      year: '2023'
    },
    {
      id: 9,
      title: 'Electromagnetism',
      des: 'Programs to analyze electromagnetism experiments.',
      img1: '/projects/2.png',
      img2: '/projects/3.png',
      iconLists: ['/bg/python-5.svg', '/bg/arduino-1.svg'],
      link: 'https://github.com/josebladex/Recursos-Python-Fisica-II',
      year: '2022'
    },
    {
      id: 10,
      title: 'EnriqueLeeApp',
      des: 'Social network platform with mobile support.',
      img1: '/projects/4.png',
      img2: '/projects/5.png',
      iconLists: [
        '/bg/android-svgrepo-com.svg',
        '/bg/kotlin-svgrepo-com.svg',
        '/bg/firebase-svgrepo-com.svg'
      ],
      link: 'https://github.com/josebladex/EnriqueLeeApp',
      year: '2022'
    },
    {
      id: 11,
      title: 'Gotarget',
      des: 'Initial landing page project for marketing.',
      img1: '/projects/6.png',
      img2: '/projects/7.png',
      iconLists: [
        '/bg/html-5-svgrepo-com.svg',
        '/bg/css-3-svgrepo-com.svg',
        '/bg/javascript-svgrepo-com.svg'
      ],
      link: 'https://www.youtube.com/watch?v=kfFfxlA0w-A',
      year: '2016'
    }
  ]
};
